...
I recently faced a situation where I had to run a piece of code only if a spcecific media query was matched, and I found a useful and simple JavaScript method that does the job. 
Which one? The `window` API method `matchMedia()`.
...
You can dig more on the `matchMedia()` method in [[1]](https://developer.mozilla.org/en-US/docs/Web/API/Window/matchMedia) and [[2]](https://www.w3.org/TR/cssom-view/#dom-window-matchmedia), 
and on the `MediaQueryList` object in [[3]](https://developer.mozilla.org/en-US/docs/Web/API/MediaQueryList).
...
In the second code block it can be argued that it would be more optimal to use the traditional `window.addEventListener('resize', someFunction)` and use `window.innerWidth` to 
...
which would be less performant than using the `matchMedia()` method to listen to a change in the specific media query.
...
#### References
[1] https://developer.mozilla.org/en-US/docs/Web/API/Window/matchMedia<br />
[2] https://www.w3.org/TR/cssom-view/#dom-window-matchmedia<br />
[3] https://developer.mozilla.org/en-US/docs/Web/API/MediaQueryList
